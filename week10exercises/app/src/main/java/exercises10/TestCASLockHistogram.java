...
class TestCASLockHistogram {
	...
	// Create an object `histogramLock` with your Histogram Lock from week 5
		final int range  = 10_000_00;
		CasHistogram casHistogram = new CasHistogram(range);
		Histogram1 histogram1 = new Histogram1(range);
	...
	// You also have the benchmarking code for Mark7
		...
		for (int c=1; c<=10; c++) {
			...
			Mark7("casHistogram with " + threadCount + " threads", i -> {
				countParallel(range, threadCount, casHistogram);
				return 0;
			});
		}
		...
		for (int c=1; c<=32; c++) {
			final int threadCount = c;
			Mark7("lock Histogram with " + threadCount + " threads", i -> {
				countParallel(range, threadCount, casHistogram);
				return 0;
			});
		}
