...
public class TestHistograms {
    ...
    public static Histogram test(Histogram histogram, int thread_count) {
        final int nums = 5000000;
        final int perThreads = nums / thread_count;
        final Thread[] threads = new Thread[thread_count];

        for (int i = 0; i < thread_count; i++) {
            final int index = i;
            final int to = ((i * perThreads) + perThreads > nums) ? nums - (i * perThreads) : perThreads;
            threads[i] = new Thread(() -> {
                for (int x = 0; x < to; x++) {
                    histogram.increment(countFactors((index * perThreads) + x));
                }
            });
        }
    ...
    public void TestCasHistogram() {
        ...
        for (int n = 0; n <= 4; n++) {
            var numThreads = (int) Math.pow(2, n);
